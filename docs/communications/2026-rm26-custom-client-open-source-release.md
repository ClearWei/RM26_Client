---
title: 【RM2026-复旦大学-星云 EGA 战队】自定义客户端与比赛模拟器开源
status: ready
...
> 本项目由复旦大学星云 EGA 战队独立开发，并非 DJI 或 RoboMaster 官方项目。RoboMaster 等名称和标识归其权利人所有。

- **开源仓库：** <https://github.com/ClearWei/RM26_Client>
- **首个版本：** 提供源码，不发布未经跨机器验收的安装包
- **主要技术：** Qt 6、C++17、QML、FFmpeg、Protobuf、MQTT

## 写在前面

最早做这套客户端时，我们想的不是“做一个开源项目”，而是下一场比赛能不能少漏掉一个状态、少做一次无意义的切换。

操作手既要控车，又要看位置、血量、资源、比赛阶段和突发事件。信息本身并不少，真正麻烦的是它们分散在不同位置，关键时刻还需要操作手自己找页面。于是我们从赛事协议接入开始，把比赛状态、战术地图、两路视频和语音提示逐步接到同一个客户端里。功能先在赛场上跑起来，赛季结束后再补模拟器、测试和文档。

下面这张画面来自一段连续对战录屏。比赛时钟、双方机器人位置和血量、事件与统计在同一个界面中持续更新；右下角辅助相机在这段素材中尚未启用，因此不把它作为本段演示结论。
...
正式赛事消息的主链路会先把裁判系统消息、机器人状态和雷达信息整理成统一比赛状态，再由指挥屏、事件提示和语音规则读取。网络层不决定页面怎么画，QML 也不直接解析 Protobuf。这样做的目的不是让界面显得更复杂，而是尽量让一项比赛事实只有一个权威状态来源。历史兼容模拟入口仍保留少量直接解析逻辑，新开发应优先使用独立模拟器和统一状态链路。
...
_图 2｜同一段连续录屏中的三个关键时刻。画面变化能说明事件、切换和恢复的先后顺序；自动触发关系由状态机实现和测试共同验证。_
...
这张图保留的是最主要的运行链路。当前 `MainWindow`、`GameData` 和 `NetworkManager` 仍然偏重，后续重构会继续收紧模块边界。独立的 `sim/` 作为外部协议对端运行；进程内模拟模块仅用于兼容已有开发流程，不作为新功能的默认入口。
...
客户端可以记录从首个 UDP 分片进入进程到完成画面绘制的端内耗时，方便在相同环境下比较改动前后的差异。公开版暂不提供缺少完整证据链的历史数字；这项指标也不包含相机采集、赛事引擎处理和上游网络传输，不能当作相机到屏幕的端到端时延。
...
配套的 `sim/` 提供本地 Web 控制台，可以修改比赛阶段、机器人状态、资源和事件，再通过 MQTT、UDP 或视频链路把数据发送给客户端。客户端仍走正常接入路径，因此同一个问题可以被重复复现，也更适合补回归测试。它不是官方赛事引擎，本地跑通也不等于现场验证通过。
...
我们希望其他战队能从中看到的，不只是一个页面长什么样，而是赛事消息怎样进入统一状态，状态怎样驱动地图、语音和视频，以及没有完整赛场环境时怎样把关键流程复现出来。

## 最短构建与验证路径

首个公开版本只提供源码。完整依赖和平台差异以仓库 [README](../../README.md) 为准，下面只保留最短入口。
...
`RoboMasterClient2025` 是当前仍在沿用的历史构建目标名，不代表协议版本。
...
首个公开版本采用 MIT License 和无内部研发历史的干净根提交。当前素材快照已经完成公开登记，
真实配置、现场地址、任务记录、调试日志和内部方案不随仓库发布。

本次验证覆盖 macOS 上的 Release 构建与 CTest，以及 GitHub Actions Ubuntu 24.04 上的仓库检查、
QML、原生构建和自动测试；Windows、Linux 桌面实机长期运行、真实裁判系统、完整网络抖动和全部
现场动作仍需要使用者结合自己的环境复核。当前只发布源码，不把开发机生成的 `.app`、容器或
模拟器 wheel 当成经过跨机器验收的发行物。
...
感谢参与客户端、模拟器、现场联调、测试和文档整理的队员，也感谢愿意公开赛季经验的 RoboMaster 社区开发者。具体贡献者署名、公开联系渠道和素材作者信息，会在取得本人同意后随正式版本一并补充。
