...
本文说明战术指挥页面现在依赖哪些真实数据、哪些内容属于本地推导，以及持久化战场时间线的
后续设计。为避免把方案误写成现状，章节会明确标注“已实现”和“尚未实现”。
