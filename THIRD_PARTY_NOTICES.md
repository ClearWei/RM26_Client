...
发行源码或二进制时，应按实际版本、构建选项和发行内容逐项复核，并提供所需的许可证文本、
版权声明、源码获取方式和修改说明。
...
模拟器依赖声明位于 `sim/pyproject.toml` 和 `sim/requirements.txt`。依赖尚未锁定精确版本，生成
锁文件和发行物时应再次核对以下许可证。
...
两份完整 MIT 文本与对应版本的上游仓库一致，并随源码保留。发行时还应确认这些版本的安全维护
状态和对应源码可获得；若改用包管理器，应同步提交锁文件。
